...
- **Azure Functions** (équivalent AWS Lambda)
- **Azure Blob Storage** (équivalent AWS S3)
...
Le code Lambda a été converti en Azure Function. Tous les fichiers sont dans `azure_function/`.

### Structure créée
```
azure_function/
├── RecommendationFunction/
│   ├── __init__.py              # Handler principal (converti depuis lambda_function.py)
...
### Différences clés AWS → Azure

| AWS Lambda | Azure Function | Changement |
|------------|----------------|------------|
| `lambda_handler(event, context)` | `main(req: func.HttpRequest)` | Signature fonction |
...
| `boto3` (S3) | `azure-storage-blob` | Client storage |
